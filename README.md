...
```
> 🟢后缀`.py`：命令行执行；🔵后缀`.sql`：在SSMS(SQL Server管理工具)打开执行

## 📋环境依赖
### 1. Python库安装
```bash
pip install pandas pyodbc python-dotenv matplotlib
```

### 2. 额外必备

- **ODBC Driver 17 for SQL Server**，pyodbc 依赖该驱动，未安装会连接失败。
...
> 
> ⚠️禁止把真实密码填写进`.env.example`，该文件会上传 GitHub。
...
> 
> 说明：创建`olist_db`数据库，9 张业务表，定义主键、物理外键约束。
> ⚠️公开原始数据集存在少量脏外键 ID，如果导入报外键冲突，SSMS 执行下面临时语句：

```
-- 临时关闭外键校验，用于导入脏数据
EXEC sp_msforeachtable 'ALTER TABLE ? NOCHECK CONSTRAINT ALL';
GO
-- 执行完python导入脚本之后，恢复约束校验
...
> 
> ⚠️脚本内部表导入顺序不能修改！维度表优先导入，满足外键依赖关系。
...
> 
> 程序连接数据库读取查询结果，matplotlib 绘图，图片保存到项目根目录。

## 📊计算的业务指标

1. 总销售额、月度订单数量变化趋势
2. 用户购买次数分布（使用`customer_unique_id`真实用户 ID，规避订单维度 ID 误区）
...
4. 删除完全重复数据行；
5. **ETL 阶段不做业务过滤**：取消、异常状态订单完整保留；业务筛选逻辑在 SQL 分析层实现，区分数据清洗与业务过滤。
